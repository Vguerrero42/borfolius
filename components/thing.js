...
const Thing = ()=>{
  ...
  useEffect(() =>{
    ...
    scene.add(group)
    // const sphere = new THREE.BoxGeometry(r*2,r*2,r*2)
    const sphere = new THREE.SphereGeometry(r)
    const object = new THREE.Mesh(sphere,new THREE.MeshBasicMaterial({color:0x0a0abf,
      transparent:true,
 ...
 */
      ...
      const segments = maxParticleCount * maxParticleCount;
        ...
        particles = new THREE.BufferGeometry();
          ...
          // particlePositions[i*-3-2] = z
					...
					particlesData.push( {
						velocity: new THREE.Vector3( - 1 + Math.random() * 1, - 1 + Math.random() * 2, - 1 + Math.random() * 1 ),
						numConnections: 0
					} );
    ...
    let sign = '+'
    const switchImage = () =>{
       currentVector = 'current image being displayed,should be a vector, 3darray'
    ...
    const animate = () => {
      ...
      if(frame <= 100 ){
        ...
        camera.zoom = -0.0125 * (101-frame/2)/2  *rotSpeed/4000
      ...
      for ( let i = 0; i < particleCount; i ++ )
          ...
          let rando = Math.random() * particleData.velocity.z - particleData.velocity.y /2
